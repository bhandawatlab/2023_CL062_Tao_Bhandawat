...
if nargin<3  % i.e. user-invoked.  
    ...
    if strcmp(get(gcf,'selectiontype'),'open')
        ...
        case 1 % line's buttondownfcn invoked
            ...
            set(h,'userdata',k);
            set(gcf,'windowbuttonmotionfcn',['moveplot(get(ax,''userdata''),''' opt ''',2)'],'doublebuffer','on');
            set(ax,'userdata',h);
            set(gcf,'windowbuttonupfcn',['moveplot(get(ax,''userdata''),''' opt ''',3)']);
        ...
        case 2 % button motion function
            ...
            if any(opt=='y')&cp(2,2)>fgy(2)
                set(ax,'ylim',[fgy(1) cp(2,2)])
            end
            if any(opt=='y')&cp(2,2)<fgy(1)
                set(ax,'ylim',[cp(2,2) fgy(2)])
            end
            if any(opt=='x')&cp(1,1)>fgx(2)
                set(ax,'xlim',[fgx(1) cp(1,1)])
            end
            if any(opt=='x')&cp(1,1)<fgx(1)
                set(ax,'xlim',[cp(1,1) fgx(2)])
            end
            
        case 3 % button up - we're done
            k=get(h,'userdata');
            set(ax,'userdata',k.axesdata); % restore axes data to its previous value
