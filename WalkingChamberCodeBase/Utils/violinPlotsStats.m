...
if ~isfield(opts,'grayscale') || (opts.grayscale == true)
    cc = 0.5*ones(nCond,3);
else
...
if opts.plotData
    try
        scatter(x,data,36,cc,'LineWidth',1);%,'k'
    catch
...
if opts.plotCentralTendency
    ...
    plot((1:size(data,2))+[-0.5; 0.5],repmat(nanmedian(data),2,1),'--k');
end
...
if opts.plotViolin
    ...
    for c = 1:nCond
        ...
        plot([tmp_f_dens;-tmp_f_dens]./m_f(c)./2+c,x_dens(c,:),'-k','linewidth',1)
    end
end
